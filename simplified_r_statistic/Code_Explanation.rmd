...
US_countydata <- US_countydata %>%
    ...
    dplyr::select(date, region, regionID = fips, cumulative_cases = cases) %>%
    mutate(region_type = "county", regionID_type = "fips")

US_statedata <- US_statedata %>%
    dplyr::select(date, region = state, cumulative_cases = positive, regionID = fips) %>%
    mutate(region_type = "state", regionID_type = "fips")
    countrydata <- countrydata %>%
    dplyr::select(date, region = location, cumulative_cases = total_cases) %>%
...
DATA = DATA %>% dplyr::select(region, region_type, regionID, regionID_type, date, cumulative_cases, INTERPROLATED)
