...
To assess the performance of this simple ratio method of estimating $R_t$, we compared it to three previously published methods of estimating $R_t$ (**Table 1**). We calculated the absolute difference in daily $R_t$ esimations between the four methods for the current SARS-CoV-2 pandemic in 10 countries, 10 US states, and 10 US counties (identified in the **Methods** section).  

```{r echo=F}
library("kableExtra")
library("tidyverse")
df = read_csv("table_1 - Description of Rt Estimation Methods.csv")
kable(df, "latex", align="l", booktabs=TRUE, escape = F, caption = "Table 1. Methods for Estimating $R_t$") %>%
...
Using baseline parameters (generation interval with mean 4 days and standard deviation of 3 days and \tau of size 7 days, based on results from [@ganyaniEstimatingGenerationInterval2020; @nishiuraCorrectingActualReproduction2010]), we find that the simple ratio method agrees well with the 3 other methods (**Figure 1**). The median error 



> **Box 1. Summary of comparison** <br>
> The 4 different methods result in very similar estimates of $R_t$. In particular when: <br>
