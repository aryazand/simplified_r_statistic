...
(with the number of successes being $n_{t+D}$ and the total number of trials being $n_{t}$ + $n_{t+D}$) using the Clopper–Pearson method, as implemented in the `poisson.test()` function in the `stats` package for the R programming language [@rcoreteamLanguageEnvironmentStatistical2020].
--->
...
# estimate r_t
calculate_rt_wrapper <- function(PARAMS, initial_simulation) {
  
  # remove incidences with NA values
  initial_simulation = initial_simulation %>% drop_na(incidence) 

  t = initial_simulation$time
  Incid = initial_simulation$incidence
  si_mean = PARAMS$e.dur + PARAMS$i.dur
  si_sd = 1
  tau = 7 
  ...
  df <- full_join(r_est, initial_simulation, by=c("date" = "time"))  
  
  return(df)
}

df <- calculate_rt_wrapper(PARAMS)
...
PARAMS.list <- map(10^c(3:7), function(x) c(PARAMS, N = x, s.num = x - i.start))
simulations <- map(PARAMS.list, simulate_seir_ode_wrapper)
simulations.df <- map2(PARAMS.list, simulations, calculate_rt_wrapper)
